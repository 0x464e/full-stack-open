...
const resolvers = {
    Query: {
        bookCount: () => books.length, authorCount: () => authors.length, allBooks: (root, args) => {
    ...
    Mutation: {
        addBook: (root, args) => {
            ...
            if (!authors.find(x => x.name === args.author)) authors = authors.concat({name: args.author, id: uuid()})
            return book
        },
